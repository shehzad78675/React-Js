...
function WishList() {
  ...
  // const subTotal = products.cart.reduce((total, item) => total + item.price * item.quantity, 0);
      ...
      <div className="p-5">
        ...
        <div className="flex mt-7 mobile:flex-col">
          <div className="flex flex-col flex-1 ">
            {/* list of products */}
            {wishListProducts.wishList.map((item, index) => {
                const {id, title, image, price, color, size} = item;
              return (
                <div key={index} className='relative'>
                   <Close className="absolute top-0 right-5 cursor-pointer" onClick={() => handleDeleteProduct(item)}/>
                    ...
                    <div className="flex self-start pl-5">
                      ...
                      <div className="flex flex-col ml-5 h-auto justify-between w-[400px]">
                        ...
                        <p className="flex items-center">
                          ...
                          <div className={`rounded-full border-2 p-[10px] cursor-pointer ${color === 'red' ? 'bg-red-600' : color === 'blue' ? 'bg-blue-600' : 'bg-yellow-400'} ml-1 w-[20px] h-[20px]`}></div>
                        </p>
                        <p>
                          <b className="mr-2">Size:</b>{size}
                        </p>
                      </div>
                    </div>
